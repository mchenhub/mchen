                                    '''
                                    从数据库中取出数据 导成Excel
                                    1、先python  ..py > 。。txt 导出成txt格式
                                    2、将txt中的文本复制到windows记事本里面
                                    3、打开excel 数据导入数据
                                    '''
#!/usr/bin/python
# -*- coding: utf-8 -*-
...
def handle_data():
    data = []
    # for i in range(1, 7):
    level_one_data = query(sql=u"SELECT * FROM xzxkProduct WHERE level = %s", list1=(1, ))
    for level_one in level_one_data:
        level_one_name = level_one.get("name")
        level_one_id = level_one.get("id")

        level_two_data = query(sql=u"SELECT * FROM xzxkProduct WHERE parent_id = %s", list1=(level_one_id, ))
        for level_two in level_two_data:
            level_two_name = level_two.get("name")
            level_two_id = level_two.get("id")

            level_three_data = query(sql=u"SELECT * FROM xzxkProduct WHERE parent_id = %s", list1=(level_two_id, ))
            ...
            if level_three_data == ():
                ...
                for level_three in level_three_data:
                    ...
                    level_four_data = query(sql=u"SELECT * FROM xzxkProduct WHERE parent_id = %s", list1=(level_three_id, ))
                    ...
                    if level_four_data == ():
                        ...
                        for level_four in level_four_data:
                            ...
                            level_five_data = query(sql=u"SELECT * FROM xzxkProduct WHERE parent_id = %s", list1=(level_four_id, ))
                            ...
                            if level_five_data == ():
                                ...
                                level_six_data = query(sql=u"SELECT * FROM xzxkProduct WHERE parent_id = %s", list1=(level_five_id, ))
                                ...
                                if level_six_data == ():
                                    ...
                                    level_seven_data = query(sql=u"SELECT * FROM xzxkProduct WHERE parent_id = %s", list1=(level_six_id, ))
